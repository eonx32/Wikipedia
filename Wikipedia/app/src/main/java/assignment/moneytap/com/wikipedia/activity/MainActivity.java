...
public class MainActivity extends AppCompatActivity implements SearchView.OnQueryTextListener, QueryManager.OnResponseListener {
    ...
    public void onResponseReceived(Response response) {
        ...
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                SuggestionAdapter suggestionAdapter = new SuggestionAdapter(getApplicationContext(), cursor, false);
    ...
    public MatrixCursor getCursor(Response response) {
        MatrixCursor matrixCursor = new MatrixCursor(new String[] {"_id", "icon", "title", "desc"});

        if(!response.isBatchComplete() || response.getQuery() == null)
            return null;

        for(Query.Page page : response.getQuery().getPages()) {
            MatrixCursor.RowBuilder rowBuilder = matrixCursor.newRow();
            rowBuilder.add("_id", page.getIndex())
